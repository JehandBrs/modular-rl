...
class MLPSimpleActor(nn.Module):
    def __init__(self, num_inputs, num_outputs):
        super(MLPSimpleActor, self).__init__()
        self.tanh = nn.Tanh()
        self.l1 = nn.Linear(num_inputs, 400)
        self.l2 = nn.Linear(400, 300)
        self.l3 = nn.Linear(300, num_outputs)
...
class MLPSimpleCritic(nn.Module):
    def __init__(self, num_inputs, num_outputs):
        super(MLPSimpleCritic, self).__init__()
        self.l1 = nn.Linear(num_inputs, 400)
        self.l2 = nn.Linear(400, 300)
        self.l3 = nn.Linear(300, num_outputs)
...
class SubprocVecEnv():
    ...
    def __init__(self, env_train_makers:makeEnvWrapper):
        ...
        self.observation_space = self.envs_train[0].observation_space
        
    def step(self, action_list):
        vect_step = [list(env_train.step(action)) for action, env_train in zip(action_list, self.envs_train)]
        return [vs[0] for vs in vect_step], [vs[1] for vs in vect_step], [vs[2] for vs in vect_step], [vs[3] for vs in vect_step]
    
    def reset(self, ):
        return [env_train.reset() for env_train in self.envs_train]
