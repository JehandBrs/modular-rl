...
def train():
    ...
    while os.path.isdir(os.path.join(DATA_DIR, exp_name)): # check if the ID alreay exist
        args.expID += 1                                    # If it's the case, change it
        exp_name = "EXP_%04d" % (args.expID)
    ...
    if not args.custom_xml:
        for morphology in args.morphologies:
            envs_train_names += [
                ...
                if (".xml" in name) and (morphology in name) and (name[:-4] in train_morphologies)
            ]
